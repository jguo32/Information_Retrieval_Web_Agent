#! /usr/local/bin/perl
#
# @(#)url_get.pl	1.22 4/11/96
# @(#)url_get.pl	1.22 /home/uts/cc/ccdc/zippy/src/perl/url_get/SCCS/s.url_get.pl
#
...
# the correct character. Note we do this *after* parsing the URL!
    ...
    if ($protocol eq "http") {
        if ($ENV{'http_proxy'} && $url !~ m/$ENV{'no_proxy'}/i) {
    ...
    if ($protocol eq "gopher") {
        if ($ENV{'gopher_proxy'} && $url !~ m/$ENV{'no_proxy'}/i) {
...
# Convert from hex. See above.
	...
	if(! $userid && ! $passwd) {
        ...
        if ($ENV{'ftp_proxy'} && $url !~ m/$ENV{'no_proxy'}/i) {
    ...
    if ($protocol eq "wais") {
        if ($ENV{'wais_proxy'} && $url !~ m/$ENV{'no_proxy'}/i) {
